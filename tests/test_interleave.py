...
from my_daemon.retrieval.interleave import EXPANSION_TEAM, SEED_TEAM, team_draft


class _ScriptedCoin:
    """A stand-in for ``random.Random`` whose tosses are known.

    Only ``random()`` is used by ``team_draft``; scripting it is the only way
    to assert on draft *order* rather than on a distribution.
    """
...
def test_team_sizes_never_differ_by_more_than_one():
    ...
    drafted = team_draft(_seeds(5), _expanded(5), rng=_ScriptedCoin(0.0, 0.99, 0.0, 0.99))
...
def test_every_candidate_is_drafted_exactly_once():
    drafted = team_draft(_seeds(4), _expanded(7), rng=_ScriptedCoin(0.0, 0.99))
...
def test_a_chunk_present_in_both_rankings_is_drafted_once():
    ...
    drafted = team_draft([shared], [shared], rng=_ScriptedCoin(0.0))
...
def test_ranking_order_within_a_team_is_preserved():
    ...
    drafted = team_draft(_seeds(3), _expanded(3), rng=_ScriptedCoin(0.0, 0.99, 0.0))
